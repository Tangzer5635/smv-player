...
function normalizeProfile(profile) {
    return {
        ...
        favoriteChannelIds: Array.isArray(profile.favoriteChannelIds)
            ? profile.favoriteChannelIds
            : [],
        ...
        settings: {
            accentColor: profile.settings?.accentColor || '#6c5ce7',
...
ipcMain.handle('profile-update', (event, {
    id,
    channels,
    stalkerSession,
    favoriteChannelIds,
...
ipcMain.handle('stalker-connect', async (event, {portalUrl, mac}) => {
    try {
        ...
        try {
            ...
            const vodGenreMap = {};
            allVodGenres.forEach((g) => {
                vodGenreMap[g.id] = g.title || g.name;
            });
...
// ── Stalker Get Stream ────────────────────────────────────────────────────────
ipcMain.handle('stalker-get-stream', async (event, {
    serverBase,
    mac,
    token,
    cmd,
